...
if __name__ == '__main__':
    if os.path.expanduser('~') == '/home/bougacha':
        ...
        spm_template_atlas_file = os.path.join(
            ...
            'bil_transformed', 'Average_atlas_invivo.nii')
        spm_dir = os.path.join(
            '/home/salma/appning_data/Pmamobipet/mrm_2010/reoriented',
            'bil_transformed')
    else:
    ...
    sammba_dir = os.path.expanduser('~/mrm_bil_transformed_preprocessed')
    sammba_raw_dir = os.path.expanduser('~/mrm_bil_transformed')
    ...
    anat_files = glob.glob(os.path.expanduser(
        '~/mrm_bil_transformed/bil_transfo_C57*.nii.gz'))
    anat_files.remove(os.path.expanduser(
        '~/mrm_bil_transformed/bil_transfo_C57_Az1_invivo.nii.gz'))
#    anat_files.remove(os.path.expanduser(
#        '~/mrm_bil_transformed/bil_transfo_C57_ab2_invivo.nii.gz'))
#    anat_files.remove(os.path.expanduser(
#        '~/mrm_bil_transformed/bil_transfo_C57_ab1_invivo.nii.gz'))
...
#                medianprops={'color':'m'})
    plt.boxplot(spm_dices, positions=np.arange(10) + .15, boxprops={'color':'r'},
                medianprops={'color':'r'})
    plt.boxplot(original_dices, positions=np.arange(10) + .3,
                boxprops={'color':'b'}, medianprops={'color':'b'})
    plt.ylabel('dice coefficient')
    plt.savefig(os.path.expanduser('~/papers/appning/poster/figures/bil_transfo_dice_boxplots.png'))
    plt.show()